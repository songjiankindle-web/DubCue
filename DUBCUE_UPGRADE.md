# DubCue Upgrade Notes

DubCue is the next-stage direction for VoxCPM Easy Launcher: a dubbing-director application that keeps VoxCPM local inference, while adding script direction, editable segmentation, rolling voice continuity, and final audio assembly.

## What changed

- Added a new `DubCue` workflow in `VoxCPM-main/app.py`.
...
When available, DubCue uses VoxCPM2 prompt-cache APIs:
...
- Speed control is engineering-level correction. VoxCPM does not expose a hard speech-rate parameter, so DubCue combines chunk sizing, pause insertion, gentle time-stretching, and segment duration guards.
