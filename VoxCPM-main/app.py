...
class VoxCPMDemo:
    ...
    def build_director_table(
        ...
        if not text:
            raise ValueError("Please paste or upload a long-form script first.")
        ...
        return segments_to_rows(segments), f"Director table ready: {len(segments)} segments."
    ...
    def generate_longform_audio(
        ...
        message = (
            ...
            f"{result.duration_seconds:.1f}s. Manifest: {result.manifest_path}"
        )
...
def create_demo_interface(demo: VoxCPMDemo):
    ...
    with gr.Blocks() as interface:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("VoxDirector Long-form"):
                with gr.Row():
                    with gr.Column(scale=1):
                        long_text = gr.Textbox(
                            label="Long script",
                            placeholder="Paste documentary narration, audiobook text, or any long script here.",
                            lines=12,
                        )
                        long_text_file = gr.File(
                            label="Or upload a UTF-8 text file",
                            file_types=[".txt", ".md"],
                        )
                        ...
                        long_prompt_text = gr.Textbox(
                            value="",
                            label="Reference transcript (optional, improves continuation cloning)",
                            lines=2,
                        )
                    ...
                    with gr.Column(scale=1):
                        ...
                        base_style = gr.Dropdown(
                            ...
                            value="documentary",
                            label="Overall narration style",
                        )
                        user_style = gr.Textbox(
                            value="",
                            label="Global style notes",
                            placeholder="e.g. calm nature documentary, slow and restrained",
                            lines=2,
                        )
                        emotion_variation = gr.Radio(
                            ...
                            value="medium",
                            label="Emotion variation",
                        )
                        allow_sentence_split = gr.Checkbox(
                            value=True,
                            label="Allow splitting very long sentences at natural pauses",
                            elem_classes=["switch-toggle"],
                        )
                ...
                with gr.Row():
                    with gr.Column():
                        with gr.Accordion("Long-form generation settings", open=False):
                            ...
                            rolling_context = gr.Slider(
                                ...
                                value=3,
                                step=1,
                                label="Rolling continuity segments",
                            )
                            use_continuity = gr.Checkbox(
                                value=True,
                                label="Keep voice continuous with rolling prompt cache",
                                elem_classes=["switch-toggle"],
                            )
                            apply_speed_control = gr.Checkbox(
                                value=True,
                                label="Apply gentle speed correction after each segment",
                                elem_classes=["switch-toggle"],
                            )
                    ...
                    with gr.Column():
                        long_audio_output = gr.Audio(label="Final long-form audio")
                        manifest_output = gr.File(label="Manifest")
